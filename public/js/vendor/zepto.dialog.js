 ...
 * 1.0.3(2016-07-15)
 * modify 2016-10-29 by zhouguanqing
 */
;(function (win, $) {
     ...
     * Private methods
     */
    var wrap, overlay, content, title, subtitle, close, cancelBtn, okBtn, delBtn, settings, timer, aTimer;
    ...
    var _renderDOM = function () {
        ...
        $('body').append(dialogWrapper = $('<div class="pop ' + settings.dialogClass + '"></div>'));
        dialogWrapper.append(
            overlay = $('<div class="bg-layer"></div>'),
            content = $('<div class="pop-wrap"></div>')
        );
        ...
        switch (settings.type) {
            case 'alert' :
                if (settings.showTitle) {
                    content.append(
                        title = $('<h4 class="dialog-content-title">' + settings.titleText + '</h4>')
                    );
                }
                content.append(
                    contentBd = $('<div class="artic-cont"></div>')
                );
                if (settings.subtitle) {
                    contentBd.append(
                        subtitle = $('<h5>' + settings.subtitle + '</h5>')
                    )
                }
                contentBd.append(
                    contentHt = $('<div>' + settings.contentHtml + '</div>')
                );
                content.append(
                    contentFt = $('<div class="operate-wrap"></div>')
                );
                contentFt.append(
                    okBtn = $('<a class="dialog-btn dialog-btn-ok ' + settings.buttonClass.ok + '" href="javascript:void(0)" style="width:100%;">' + settings.buttonText.ok + '</a>')
                );
                break;
            ...
            case 'confirm':
                ...
                contentFt.append(
                    cancelBtn = $('<a class="dialog-btn dialog-btn-cancel ' + settings.buttonClass.cancel + '"  href="javascript:void(0)">' + settings.buttonText.cancel + '</a>'),
                    okBtn = $('<a class="dialog-btn dialog-btn-ok ' + settings.buttonClass.ok + '"  href="javascript:void(0)">' + settings.buttonText.ok + '</a>')
                );
                break;
            ...
            case 'tips' :
                ...
                var tipsContent = settings.infoText;

                content.append(
                    //contentBd = $('<div class="dialog-content-bd" >' + tipsContent + '</div>')
                    contentBd = $('<div class="dialog-content-bd">' + '<p>'+tipsContent +'</p>'+'</div>')
                );
    ...
    $.fn.dialog.defaults = {
        ...
        infoIcon: location.origin+'/images/loading.gif',      // working in info type
